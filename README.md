# Backend de Tareas con FastAPI y Railpack 🚀

Esta es una aplicación backend simple para gestionar tareas, construida con FastAPI y configurada para ser desplegada fácilmente usando **Railpack** en plataformas como Dokploy.

## Características

- 🚀 **FastAPI**: Alto rendimiento y fácil de usar.
- 🗄️ **PostgreSQL**: Persistencia de datos robusta (vía SQLAlchemy Async).
- 📦 **Railpack**: Construcción automática y optimizada de contenedores.
...
## Despliegue en Dokploy (vía Railpack)

Esta configuración utiliza **Railpack** para construir la imagen Docker de manera eficiente y sin configuración compleja.

### Paso 1: Configuración en Dokploy

1.  Asegúrate de que tu proyecto en Dokploy esté configurado para usar **Railpack** (o Nixpacks, ya que Railpack es compatible/sucesor en muchos contextos, o simplemente Dockerfile si se autogenera).
2.  Railpack detectará automáticamente `requirements.txt` y `railpack.json`.

### Paso 2: Despliegue

Simplemente haz push de tus cambios a tu repositorio. Dokploy (con Railpack/Nixpacks habilitado) se encargará de:

1.  Detectar que es una aplicación Python.
2.  Instalar las dependencias de `requirements.txt`.
3.  Usar el comando de inicio definido en `railpack.json`: `uvicorn app.main:app --host 0.0.0.0 --port $PORT`.

No necesitas construir imágenes manualmente ni configurar Dockerfiles complejos.

> **Nota**: Si Dokploy no soporta Railpack nativamente aún, suele ser compatible con la configuración de Nixpacks o Buildpacks estándar que leerán el `requirements.txt`.
